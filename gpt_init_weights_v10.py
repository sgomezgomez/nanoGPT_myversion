...
## ----------------------------------
## Hyperparameters
batch_size = 128     # Number of independent sequences to process in parallel
...
class MultiHeadAttention(nn.Module): # Multi head attention
    ...
    def forward(self, x):
        ...
        out = self.proj(out) # Projection layer, Linear transformation of the outcome from the previous layer
...
log_text((str (sum(p.numel() for p in m.parameters())/1e6) + ' Million of parameters'))
